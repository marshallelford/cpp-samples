...
public:
    char slowestKey(vector<int>& releaseTimes, string keysPressed) {
        ...
        for (int i = 0; i < keysPressed.size(); i++) {
            if (i == 0) {
                largestTime = releaseTimes[i];
                largestKey = keysPressed[i];
            } else {
                if (releaseTimes[i] - releaseTimes[i-1] == largestTime) {
                    largestKey = max(largestKey, keysPressed[i]);
                } else if (releaseTimes[i] - releaseTimes[i-1] > largestTime) {
                    largestTime = releaseTimes[i] - releaseTimes[i-1];
                    largestKey = keysPressed[i];
                }
            }
        }
